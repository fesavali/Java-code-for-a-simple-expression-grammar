...
public class tally {
    ...
    //    <factor>   -> '(' <expr> ')' | '-' <factor>
    //                     | identifier     | number
    //since we cannot have two functions by name function i renamed this to factor_sol
    // ************************************************
        ...
        while (token == (int) '^') {
            ...
            if( power!=0) {
                if ((int) saveOp == '^') {
                    ...
                    System.out.println("error: " + (char) saveOp + "  expected '^' ");
                }
            }else{
                System.out.println("error: " + (char) saveOp + "  expected factor to be greater than 0");
            }
            s.push_op(saveOp);
        }
        s.indentPrint(-1, "factor", print);
    ...
    //<mult_op> -> '*' | '/' | ‘%’
    // ************************************************
    private static int mult_op() {
        s.indentPrint(1, "mult_op", print);
        int subtotal = 0;
        while (token == (int) '*' || token == (int) '/' || token == (int) '%') {
            int saveOp = token;
            token = lex.getToken();
            if ((int) saveOp == '*') subtotal *= term();
            else if ((int) saveOp == '/') subtotal = subtotal / term();
                ...
                System.out.println("error: " + (char) saveOp + "  expected '*'  or '/' or '%' ");
            }
            s.push_op(saveOp);
        }
        s.indentPrint(-1, "mult_op", print);
...
class Lexical {
    ...
    public Lexical(String argv[]) {
        try {
            ...
            tokenstream.ordinaryChar('-');
            tokenstream.ordinaryChar('/');
            tokenstream.ordinaryChar('^'); //add symbol for power
            tokenstream.ordinaryChar('%'); //add modulus operator
        } catch (IOException e) {
...
class Node {
    ...
    static boolean isOperator(String s) {
        ...
        if (s.contains("/")) return true;
        if (s.contains("^")) return true;
        if (s.contains("%")) return true;//add modulus operator
        return false;
    }
}
